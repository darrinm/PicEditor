...
package util
{
	...
	public class PicnikFont implements IDocumentSerializable
	{
		...
		// isEmbeded (yes, misspeled) doesn't actually mean that the font is embedded,
		// but that it is not loaded from an external SWF. By this redefinition, device
		// fonts have isEmbeded = true (surprise!)
		...
		private static var _fntUniversal:PicnikFont;
		...
		public static function Universal(): PicnikFont {
			if (_fntUniversal == null) {
				_fntUniversal = new PicnikFont();
				_fntUniversal.familyName = "_sans";
				_fntUniversal.baseFileName = "";
				_fntUniversal.isDevice = true;
				_fntUniversal.isEmbeded = true; // I.e. not loaded from an external SWF
			}
			return _fntUniversal;
		}

		public static function Default(): PicnikFont {
			// If the user's locale is one that we only have one good font for, make it the default.
			if (['jp_JP', 'ko_KR', 'ru_RU', 'vi_VN', 'zh_CN', 'zh_HK'].indexOf(CONFIG::locale) != -1) {
				if (_fntDefault == null)
					_fntDefault = PicnikFont.Universal();
				return _fntDefault;				
			}
		...
		private function get swfUrl(): String {
			if (isEmbeded) return "";
			return PicnikBase.StaticUrl(FontManager.FontsBasePath() + "swfs/" + baseFileName + ".swf");
		}
