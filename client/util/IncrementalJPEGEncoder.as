...
// Our primary extension is to allow the encoder to be used in an incremental
// fashion. It returns immediately from the call to encode() and the caller
// must repeatedly call continueEncode() until the whole encoding is finished.

// New methods:
// continueEncode(): ByteArray
// incrementalEncode(): ByteArray
//
// New properties:
// progress -- 0 - 100
//
// New events:
// ProgessEvent.PROGRESS
// Event.COMPLETE
 ...
 */
public class IncrementalJPEGEncoder extends EventDispatcher implements IImageEncoder
{
//    include "../../core/Version.as";
     ...
     */
    public function IncrementalJPEGEncoder(quality:Number = 50.0)
    {
    	super();
    ...
    public function incrementalEncode(bitmapData:BitmapData):ByteArray
    {
        return internalEncode(bitmapData, bitmapData.width, bitmapData.height,
							  bitmapData.transparent, true);
    }
	...
	private function internalEncode(source:Object, width:int, height:int,
									transparent:Boolean = true, fIncremental:Boolean=false):ByteArray
    {
    	this.width = width;
    	this.height = height;
    	this.ypos = 0;
    ...
    public function continueEncode(): ByteArray {
    	if (ypos >= height)
    		return byteout;

		// Encoding two rows of blocks at a time better fills the Timer slice
		_encode(2);
	   
        if (ypos >= height)
			return finish();

		dispatchEvent(new ProgressEvent(ProgressEvent.PROGRESS, false, false, ypos, height));
		return null;
    }
   
    private function _encode(cIterations:int=int.MAX_VALUE): void {
        for (var c:int = 0; c < cIterations && ypos < height; c++, ypos += 8) {
    ...
    private function finish(): ByteArray {
