...
package imagine.documentObjects {
	...
	public class PhotoGrid extends DocumentObjectBase {
		...
		private var _nRoundedPct:Number = 0.0;
		...
		// TODO(darrinm): who says this is obsolete? It seems to be what we're using.
		...
		public override function get serializableProperties(): Array {
			return super.serializableProperties.concat([
				"numRows", "numColumns", "gap", "backgroundColor", "backgroundAlpha", "templateName", "roundedPct",
		...
		[Bindable]
		public function set roundedPct(nRoundedPct:Number): void {
			_nRoundedPct = nRoundedPct;
			Invalidate();
		}
		...
		private function CopyTargetDimensions(tgtSrc:Target, tgtDest:Target): void {
			const kstrCopyProps:Array = ["x", "y", "scaleX", "scaleY", "rotation", "circular", "roundedPct"];
		...
		private function LocalizeText(dobc:DocumentObjectContainer): void {
			...
			if (LocaleInfo.UsingSystemFont()) return;
			
			var txt:imagine.documentObjects.Text = (dobc as imagine.documentObjects.Text);
			if (txt != null) {
				txt.text = LocUtil.EnglishToLocalized(txt.text);
			} else {
				// Recurse
