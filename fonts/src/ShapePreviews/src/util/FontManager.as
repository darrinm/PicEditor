...
package util
{
	...
	public class FontManager
	{
		...
		public static function FindFontFamilyByName(strName:String): FontFamily {
			if (_fm._afntf != null) {
				var fntf:FontFamily;
				for each (fntf in _fm._afntf) {
					if (fntf.name == strName) return fntf;
				}
		...
		// Returns null if the list isn't loaded yet, or the family is not found.
		public static function FindFontByName(strName:String, fBold:Boolean, fItalic:Boolean, fFallback:Boolean=false): PicnikFont {
			var fnt:PicnikFont = null;
			var fntf:FontFamily = FindFontFamilyByName(strName);
			if (fntf) {
		...
		private function _GetFontList(fnComplete:Function): void {
			...
			if (_xmlFontList) {
				Application.application.callLater(CallFontListLoadedCallbacks);
		...
		private function LoadFontList(): void {
			...
			var urlr:URLRequest = new URLRequest(FontManager.FontsXMLBasePath() + "fonts.xml?rel=123");
